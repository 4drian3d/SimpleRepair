...
public final class SimpleRepairFabric implements ModInitializer, RepairLogic<ServerPlayer, InteractionHand> {
  ...
  private void registerRepairCommand(String name, CommandDispatcher<CommandSourceStack> dispatcher, Commands.CommandSelection environment) {
    final LiteralArgumentBuilder<CommandSourceStack> cmd = literal(name)
        .requires(this.requirePermission(environment, "simplerepair.use"))
        .then(argument("hand", StringArgumentType.word())
            .requires(this.requirePermission(environment, "simplerepair.use.handtype"))
            ...
            .then(argument("percentage", DoubleArgumentType.doubleArg(1, 100))
                .requires(this.requirePermission(environment, "simplerepair.use.percentage"))
                .executes(context -> {
  ...
  private Predicate<CommandSourceStack> requirePermission(final Commands.CommandSelection environment, final String permission) {
    return environment == Commands.CommandSelection.INTEGRATED
        ? src -> src.isPlayer() && Minecraft.getInstance().hasSingleplayerServer()
        : src -> src.isPlayer() && Permissions.check(src, permission, 2);
  }
}
